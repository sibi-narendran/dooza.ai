...
export function dbToPost(row) {
    ...
    // Same-site assets are stored as absolute URLs in some legacy rows. Convert
    // them to paths so next/image treats them as local rather than remote.
    const image = row.image_url?.startsWith('https://www.dooza.ai/')
        ? row.image_url.replace('https://www.dooza.ai', '')
        : row.image_url || null;
    return {
        id: row.id,
        title: row.title,
